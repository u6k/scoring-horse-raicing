...
module ScoringHorseRacing::Parser
  class RaceListPageParser < Crawline::BaseParser
    ...
    def redownload?
      @logger.debug("RaceListPageParser#redownload?: start: Date.today.prev_month(3)=#{Date.today.prev_month(3)}, @date=#{@date}")

      (Date.today.prev_month(3) < @date)
    end

    def valid?
      ((not @related_links.empty?) && (not @race_id.nil?) && (not @date.nil?) && (not @course_name.nil?))
    end
