  ...
  describe "#parse" do
    context "2018-06-24 hanshin 1R odds win page" do
      it "is odds win info" do
        ...
        expected_win = build_expected_win
        expected_place = build_expected_place
        expected_bracket_quinella = build_expected_bracket_quinella

        expect(InvestmentHorseRacing::Crawler::Model::OddsWin.all).to match_array(expected_win)
        expect(InvestmentHorseRacing::Crawler::Model::OddsPlace.all).to match_array(expected_place)
        expect(InvestmentHorseRacing::Crawler::Model::OddsBracketQuinella.all).to match_array(expected_bracket_quinella)
      end
      ...
      it "overwrite when existing data" do
        ...
        expected_win = build_expected_win
        expected_win << have_attributes(race_meta_id: meta_11111.id, horse_number: 1, odds: 111.11)
        expected_win << have_attributes(race_meta_id: meta_22222.id, horse_number: 2, odds: 222.22)

        expected_place = build_expected_place
        expected_place << have_attributes(race_meta_id: meta_11111.id, horse_number: 1, odds_1: 111.11, odds_2: 111.22)
        expected_place << have_attributes(race_meta_id: meta_22222.id, horse_number: 2, odds_1: 222.11, odds_2: 222.22)

        expected_bracket_quinella = build_expected_bracket_quinella
        expected_bracket_quinella << have_attributes(race_meta_id: meta_11111.id, bracket_number_1: 1, bracket_number_2: 1, odds: 111.11)
        expected_bracket_quinella << have_attributes(race_meta_id: meta_22222.id, bracket_number_1: 2, bracket_number_2: 2, odds: 222.22)
    ...
    def build_expected_bracket_quinella
      [
