...
class RaceListPageTest < ActiveSupport::TestCase
  ...
  test "download" do
    ...
    # execute - download
    race_list_pages.each { |r| r.download! }
    ...
    # execute - re-download
    race_list_pages_2.each { |r| r.download! }
  ...
  test "download: case invalid html" do
    ...
    date = Time.zone.local(1900, 1, 1)
    course_name = "aaa"

    # execute
    race_list_page = RaceListPage.new(url, date, course_name)

    # check
    assert_equal 0, RaceListPage.find_all.length

    assert_equal url, race_list_page.url
    assert_equal date, race_list_page.date
    assert_equal course_name, race_list_page.course_name
    assert_not race_list_page.exists?
    assert_not race_list_page.valid?

    # execute
    race_list_page.download!
